...
class SaleOrderLine(models.Model):
	...
	def compute_on_pf(self):
		for rec in self:
			...
			currency_id = rec.order_id.currency_id if rec.order_id.currency_id else self.env.company.currency_id

			discount = currency_id.round(price_subtotal * rec.discount / 100)
			price_subtotal = currency_id.round(price_subtotal - discount)
			rec.pf_gst_amount = currency_id.round(price_subtotal * rec.pf_gst_per / 100)
	...
	def _compute_amount(self):
		for line in self:
			...
			currency_id = line.order_id.currency_id if line.order_id.currency_id else self.env.company.currency_id

			price_subtotal = line.price_unit * line.product_uom_qty
			discount = currency_id.round(price_subtotal * line.discount / 100)
			price_subtotal = currency_id.round(price_subtotal - discount)
			
			pf_gst_amount = currency_id.round(price_subtotal * line.pf_gst_per / 100)
			
			line.price_subtotal = currency_id.round(
				base_line['tax_details']['raw_total_excluded_currency'] + pf_gst_amount
			)
			line.price_total = currency_id.round(
...
class SaleOrder(models.Model):
	...
	def _compute_amounts(self):
		...
		for order in self:
			...
			currency_id = order.currency_id if order.currency_id else self.env.company.currency_id
			order.currency_id = currency_id.id
			...
			order.amount_tax = order.currency_id.round(tax_totals['tax_amount_currency'] + gst_amount + gst_amount_line)
			...
			for subtotal in tax_totals['subtotals']:
				...
				if subtotal.get('tax_groups'):
					for tax_group in subtotal['tax_groups']:
						gst_amount_line_wise = order.get_total_tax_on_gst(tax_id=tax_group.get('id'))
						tax_group['tax_amount'] = tax_group['tax_amount'] + gst_amount_line_wise
						tax_group['tax_amount_currency'] = tax_group['tax_amount_currency'] + gst_amount_line_wise

			tax_totals['base_amount_currency'] = tax_totals['base_amount_currency'] + gst_amount + gst_amount_line
			tax_totals['total_amount_currency'] = tax_totals['total_amount_currency'] + gst_amount + gst_amount_line
			tax_totals['base_amount'] = tax_totals['base_amount'] + gst_amount + gst_amount_line
			tax_totals['total_amount'] = tax_totals['total_amount'] + gst_amount + gst_amount_line
